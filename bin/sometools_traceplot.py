...
import plot_pygmt as PG

DATASTORE = "/Users/matteo/polybox/PyGMT_DATA.032021/TUNING"

if len(sys.argv) < 2:
    print("USAGE: %s TRACEPATH [GLOBAL-Json]" % Path(sys.arg[0]).name)
    sys.exit()

eqid, stat = sys.argv[1], sys.argv[2]
...
for xx in glob.glob(os.sep.join([MAINDIR, "waveforms", eqid, "*."+eqid+"*."+stat+"*.SAC"])):
    _de = read(xx)[0]
    st += _de
